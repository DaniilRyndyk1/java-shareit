...
public interface ItemRepository extends JpaRepository<Item, Long> {
    ...
    @Query(value = """
            select b
            from Booking b
            where b.id in (SELECT b2.id
            FROM Booking b2
            WHERE b2.item.id = :itemId and EXTRACT(epoch FROM b2.start - NOW()) > 0 and status <> 'REJECTED'
            order by EXTRACT(epoch FROM b2.start - NOW()) desc)""")
    ...
    @Query(value = """
            ...
            where b.id in (SELECT b2.id
            FROM Booking b2
            WHERE b2.item.id = :itemId and EXTRACT(epoch FROM b2.start - NOW()) <= 0
            order by EXTRACT(epoch FROM b2.start - NOW()) desc)""")
